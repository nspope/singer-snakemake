...
The input files for each chromosome are:
  ...
  - __\<chromosome_name>.filter.txt__ (optional) text file containing 1-based positions of omitted SNPs (one per line). These may be present in the VCF (in which case they are removed by the pipeline) or not (in which case they are still used to calculate an adjustment to mutation rate, see below). For example, these might be SNPs that could not be called accurately in some samples, or where the ancestral state could not be reliably determined. These SNPs will not be used to estimate the ARG.
  - __\<chromosome_name>.sample.mask.bed__ (optional) bed file containing inaccessible intervals for individual haplotypes (one per line). The fourth column should be the sample name. These are only used if `max-masked-samples` is set to greater than 0 in `config.yaml`. Sites missing in these individuals will be imputed based on allele frequency, and then the specified haplotype will be pruned from the inferred trees in these regions. 
...
max-missing: 0.75 # ignore chunks with more than this proportion of masked sequence or filtered variants
...
singer-mcmc-thin: 100 # thinning interval between MCMC samples
...
singer-skip-failures: false # treat chunks where MCMC fails as missing
# --- POLEGON options ---
...
The output files for each chromosome will be generated in `results/<chromosome_name>`:
  ...
  - __\<chromosome_name>.mut_rate.p__ : `msprime.RateMap` containing fine-scale adjusted mutation rates
  - __\<chromosome_name>.inaccessible.p__ : `msprime.RateMap` containing fine-scale proportion of inaccessible bases
...
because `tskit.TreeSequence` and `msprime.RateMap` are in 0-based coordinates. So, for example: `tskit.TreeSequence.sites_position` will
match the input positions from the VCF; `rec_rate.get_rate(variant_position)` will
give the correct recombination rate for a given position from the input VCF; and `rec_rate.get_rate(0)` is `NaN` (and
will never correspond to a variant position).
