...
The output files for each chromosome will be generated in `results/<chromosome_name>`:
  ...
  - __plots/pair-coalescence-pdf.png__ : pair coalescence time distribution for all samples, with a thin line for each MCMC replicate and a thick line for the posterior mean
  - __plots/cross-coalescence-pdf.png__ : pair coalescence time distribution within and between strata (if supplied) 
